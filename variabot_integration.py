...
class ModelIntegrationManager:
    ...
    def __init__(self, config: IntegrationConfig):
        ...
        if self.capabilities.platform in [PlatformType.ANDROID_TERMUX, PlatformType.ANDROID_NATIVE]:
    ...
    def register_existing_bot_integration(self, bot_files: List[str]):
        ...
        for bot_file in bot_files:
            ...
            integration_info = {
                ...
                'chat_interface': 'chat_message' in content or 'st.chat_input' in content,
                'history_logging': 'writehistory' in content,
                'compatible': True
            }
            
            integrated_bots[bot_name] = integration_info
            logger.info(f"Registered bot: {bot_name} - {integration_info['model_type']}")
    ...
    def resource_managed_execution(self):
        ...
        try:
            ...
            if memory_used > self.config.memory_limit_mb:
                logger.warning(f"Memory usage {memory_used}MB exceeded limit {self.config.memory_limit_mb}MB")
    ...
    async def async_model_execution(self, model_call: Callable, *args, **kwargs):
        ...
        with self.resource_managed_execution():
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
                result = await loop.run_in_executor(executor, model_call, *args, **kwargs)
...
def initialize_integration(config: Optional[IntegrationConfig] = None) -> ModelIntegrationManager:
    ...
    if config is None:
        ...
        config = IntegrationConfig(
            ...
            android_optimized=capabilities.platform in [PlatformType.ANDROID_TERMUX, PlatformType.ANDROID_NATIVE],
            termux_mode=capabilities.termux_available,
            max_concurrent_models=2 if capabilities.memory_gb < 4 else 4,
            memory_limit_mb=int(capabilities.memory_gb * 512) if capabilities.memory_gb > 0 else 1024,
            auto_scaling=True,
