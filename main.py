...
class VariabotUniversalInterface:
    ...
    def _select_optimal_bot(self, model_preference: str = "auto") -> Optional[str]:
        ...
        for bot_name in priorities:
            if (
                bot_name in self.integrated_bots
                and self.integrated_bots[bot_name]["compatible"]
            ):
                return bot_name
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="VARIABOT Universal Multi-Platform Interface"
    )
    parser.add_argument(
        "--interface",
        "-i",
    ...
    parser.add_argument("--model", "-m", default="auto", help="Specific model to use")
    parser.add_argument(
        "--android-optimize", action="store_true", help="Force Android optimizations"
    )
